...
export class MockInterCarsProvider extends SupplierProvider {
  async searchProducts(query = "") { return products.filter((product) => `${product.brand} ${product.name} ${product.sku}`.toLowerCase().includes(query.toLowerCase())).flatMap((product) => product.supplierOffers.filter((offer) => offer.supplier === "intercars")); }
  async getProduct(product) { return product.supplierOffers.find((offer) => offer.supplier === "intercars") || null; }
